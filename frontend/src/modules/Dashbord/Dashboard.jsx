...
import tutorialsdev from '../../assets/OMEN.png'
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchConversations = async () => {
            const res = await fetch(`http://localhost:9876/api/conversation/${loggedInUser?.id}`, {
                method: 'GET',
                headers: {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            const res = await fetch(`http://localhost:9876/api/user/${user?.id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            // console.log(resData)
            // console.log("Couocu"+user?.id)
        }
        fetchUsers()
    }, [])

    const fetchMessages = async (conversationId, receiver) => {
        const res = await fetch(`http://localhost:9876/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
    ...
    const fetchMessages1 = async (conversationId, receiver) => {
        const res = await fetch(`http://localhost:9876/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
    ...
    const sendMessage = async (e) => {
        ...
        const res = await fetch(`http://localhost:9876/api/message`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    return (
        <>
            <div className='w-screen flex '>
                <div className='w-[25%] h-screen dashboard__color overflow-scroll'>
                    ...
                    <div className='mx-14 mt-10'>
                        <div className='text-color text-lg'>Messages</div>
                        <div>
                            {
                    ...
                    {/* separation */}
                <div className='w-[50%] h-screen dashboard__main flex flex-col items-center'>
                    {
                                ...
                                messages?.messages?.length > 0 ?
                                    messages.messages.map(({ message, user: { id } = {} }) => {
                                        return (
                                            <span key={message}>
                                                <>
                                                    <div className={`max-w-[40%] rounded-b-xl p-4 mb-6 ${id === user?.id ? ' dashboard__color1 text-white rounded-tl-xl ml-auto' : ' dashboard__color3 rounded-tr-xl'} `}>{message}</div>
                ...
                {/* separation */}
                <div className='w-[25%] h-screen  dashboard__color px-8 py-16 overflow-scroll'>
                    <div className='text-color text-lg'>People</div>
                    <div>
                        {
                            users.length > 0 ?
                                users.map(({ userId, user }) => {
                                    return (
                                        <span key={userId}>
                                            <div className='flex items-center py-8 border-b border-b-gray-300'>
                                                <div className='cursor-pointer flex items-center' onClick={() => fetchMessages1('new', user)}>
                                                    <div><img src={Img1} className="w-[60px] h-[60px] rounded-full p-[2px] border border-primary" /></div>
                                                    <div className='ml-6'>
                                                        <h3 className='text-lg font-semibold'>{user?.fullName}</h3>
                                                        <p className='text-sm font-light text-gray-600'>{user?.email}</p>
